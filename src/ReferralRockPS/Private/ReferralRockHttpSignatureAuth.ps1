...
.OUTPUTS
    Hashtable
#>
function Get-ReferralRockHttpSignedHeader {
    param(
        [string]$Method,
    ...
    $httpSigningConfiguration = Get-ReferralRockConfigurationHttpSigning
    $Digest = $null

    #get the body digest
    $bodyHash = Get-ReferralRockStringHash -String $Body -HashName $httpSigningConfiguration.HashAlgorithm
    ...
    foreach ($headerItem in $httpSigningConfiguration.HttpSigningHeader) {
        ...
        if ($headerItem -eq $HEADER_REQUEST_TARGET) {
            ...
            $created = Get-ReferralRockUnixTime -Date $dateTime -TotalTime TotalSeconds
            ...
            $expireEpocTime = Get-ReferralRockUnixTime -Date $expire -TotalTime TotalSeconds
    ...
    $signatureHashString = Get-ReferralRockStringHash -String $headerValuesString -HashName $httpSigningConfiguration.HashAlgorithm

    #Gets the Key type to select the correct signing algorithm
    $KeyType = Get-ReferralRockKeyTypeFromFile -KeyFilePath $httpSigningConfiguration.KeyFilePath

    if ($keyType -eq "RSA") {
        $headerSignatureStr = Get-ReferralRockRSASignature -PrivateKeyFilePath $httpSigningConfiguration.KeyFilePath `
        ...
        $headerSignatureStr = Get-ReferralRockECDSASignature -ECKeyFilePath $httpSigningConfiguration.KeyFilePath `
            -DataToSign $signatureHashString `
            -HashAlgorithmName $httpSigningConfiguration.HashAlgorithm `
            -KeyPassPhrase $httpSigningConfiguration.KeyPassPhrase
    }
    #Deprecated
    <#$cryptographicScheme = Get-ReferralRockCryptographicScheme -SigningAlgorithm $httpSigningConfiguration.SigningAlgorithm `
...
.OUTPUTS
    Base64String
#>
function Get-ReferralRockRSASignature {
    Param(
    ...
    try {
        ...
        if ($PSVersionTable.PSVersion.Major -ge 7) {
            ...
            $rsa_provider_path = Join-Path -Path $PSScriptRoot -ChildPath "ReferralRockRSAEncryptionProvider.cs"
...
.OUTPUTS
    Base64String
#>
function Get-ReferralRockECDSASignature {
    param(
...
.Outputs
    String
#>
Function Get-ReferralRockStringHash {
    param(
        [Parameter(Mandatory = $true)]
        [AllowEmptyString()]
        [string]$String,
...
Integer
#>
function Get-ReferralRockUnixTime {
    param(
        [Parameter(Mandatory = $true)]
        [DateTime]$Date,
        [Parameter(Mandatory = $false)]
...
function Get-ReferralRockCryptographicScheme {
    param(
        [Parameter(Mandatory = $true)]
        [string]$SigningAlgorithm,
        [Parameter(Mandatory = $true)]
...
String
#>
function Get-ReferralRockKeyTypeFromFile {
    param(
        [Parameter(Mandatory = $true)]
        [string]$KeyFilePath
    )
